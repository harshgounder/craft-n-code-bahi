...
def hint_flags(chain):
    ...
    for mid, raw_evs in _meetings_with_events(chain):
        ...
        unique_sigs = {w["sig"] if isinstance(w, dict) else w for w in ws}
        if len(unique_sigs) < MIN_WITNESSES:
            flags.append({"hint": "missing_witness", "meeting": mid,
                          "evidence": "close signed by %d distinct witness(es)" % len(unique_sigs)})
